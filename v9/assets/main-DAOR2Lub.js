import{A as X,M as w}from"./audio-system-IwvVicxu.js";//! 定型文の型定義。
//! ランダムQSO生成用データ。
...
function z(){const a=G(),t=G(),e=O(Y),s=O(Y),n=O($),i=O($),l=O(W),o=O(W),r=O(P),R=O(P),A=O(["GM","GA","GE","GN"]),u=`CQ CQ CQ DE ${a} ${a} PSE K BT ${a} DE ${t} ${t} K BT R ${t} DE ${a} ${A} OM TNX FER UR CALL BT UR RST IS ${o} ${o} BT MI QTH IS ${e} ${e} ES MI NAME IS ${n} ${n} HW ? AR ${a} DE ${a} KN BT R ${a} DE ${t} ${A} DR ${n} OM TKS FER FB RPT ${o} FM ${e} BT UR RST ALSO ${l} ${l} VY FB MI QTH IS ${s} ${s} BT NAME IS ${i} ${i} HW? ${a} DE ${t} KN BT R FB DE ${a} DR ${i} OM BT MI RIG IS ${r} PWR 100W BT PSE UR QSL CRD VIA JARL ? MI CRD SURE HW? AR ${t} DE ${a} KN BT R ${a} DE ${t} OK ${n} OM BT UR RIG ${r} VY FB BT MI RIG IS ${R} BT QSL VIA JARL OK SURE BT TNX FB QSO ES 73 AR ${a} DE ${t} VA BT OK ${i} SOLID CPI BT TKS FB QSO ES BEST 73 AR ${t} DE ${a} VA TU E E`;return{id:`qso-random-${Date.now()}`,category:"qso",title:"ランダムQSO",content:u}}//! ラバースタンプQSOのサンプルデータ。
...
d.load();const t=d.getAll();//! LocalStorageから最後に選択したカテゴリーを復元。
const e=localStorage.getItem("v8.listening.category");e&&["qso","text100","text200","text300","custom"].includes(e)&&(this.state.currentCategory=e);//! AudioSystemを初期化。
...
document.querySelectorAll(".tab-button").forEach(e=>{e.addEventListener("click",()=>{const s=e.getAttribute("data-category");if(s){this.state.currentCategory=s,this.state.selectedTemplate=null,this.state.showResult=!1,this.state.showAnswer=!1,this.state.showDialogFormat=!1,this.state.userInput="";//! LocalStorageに選択したカテゴリーを保存。
localStorage.setItem("v8.listening.category",s),this.render()}})});//! 定型文選択ボタン。
...
document.getElementById("saveSettings")?.addEventListener("click",()=>{const o=document.getElementById("characterSpeed"),r=document.getElementById("effectiveSpeed"),R=document.getElementById("frequency"),A=parseInt(o.value);let u=parseInt(r.value);//! 実効速度は文字速度を上限とする。
u>A&&(u=A,r.value=A.toString()),d.set("characterSpeed",A),d.set("effectiveSpeed",u),d.set("frequency",parseInt(R.value)),d.set("volume",parseInt(i.value)/100);//! AudioSystemを更新。
...
const R=r.map(I=>I.morse).join(" / ");//! タイミングを計算（ミリ秒）。
const A=1200/e,u=A*3,c=A,g=A*3,p=7*(1200/s);//! 総時間を計算。
let E=0;for(let I=0;I<R.length;I++){const T=R[I];T==="."?E+=A+c:T==="-"?E+=u+c:T===" "?E+=g-c:T==="/"&&(E+=p-c)}//! サンプルレート。
const m=44100,S=E/1e3;//! OfflineAudioContextを作成。
const H=new OfflineAudioContext(1,Math.ceil(m*S),m);//! モールス信号を生成（セグメントごとに周波数を変える）。
...
g(36,"data"),c.setUint32(40,R,!0);//! PCMデータを書き込む。
let p=44;for(let E=0;E<r.length;E++){const m=Math.max(-1,Math.min(1,r[E])),S=m<0?m*32768:m*32767;c.setInt16(p,S,!0),p+=2}return new Blob([u],{type:"audio/wav"})}destroy(){this.audioSystem.stopContinuousTone()}}export{at as ListeningTrainer};
