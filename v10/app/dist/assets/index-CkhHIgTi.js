...
const u=document.getElementById("left-key-binding");u&&(u.addEventListener("click",()=>{u.value="キーを押してください...",u.classList.add("waiting-key")}),u.addEventListener("keydown",h=>{h.preventDefault(),h.stopPropagation(),u.value=h.code,u.classList.remove("waiting-key")}));//! 右パドルキーバインド設定。
const m=document.getElementById("right-key-binding");m&&(m.addEventListener("click",()=>{m.value="キーを押してください...",m.classList.add("waiting-key")}),m.addEventListener("keydown",h=>{h.preventDefault(),h.stopPropagation(),m.value=h.code,m.classList.remove("waiting-key")}))}closeSettingsModal(t){const e=document.getElementById("settings-modal");if(!e)return;//! モーダルを非表示。
if(e.classList.remove("active"),t){//! 設定を適用。
...
const u=document.getElementById("current-wpm");u&&(u.textContent=c.toString())}if(a){this.iambicMode=a.value;//! IambicモードをlocalStorageに保存。
...
const t=await fetch(p);if(!t.ok)throw new Error(`Failed to load flashcard data: ${t.statusText}`);const e=await t.text();return Z(e)}function Z(p){//! 行に分割。
const t=p.split(`
...
const l=document.getElementById("frequency-input");l&&l.addEventListener("input",()=>{const h=parseInt(l.value);this.audio.setFrequency(h)});//! WPM。
const o=document.getElementById("wpm-input");o&&o.addEventListener("input",()=>{const h=parseInt(o.value);this.audio.setWPM(h)});//! テスト再生。
const c=document.getElementById("test-morse-btn");c&&c.addEventListener("click",async()=>{await this.playMorse("CQ")});//! キャンセルボタン。
const u=document.getElementById("cancel-btn");u&&u.addEventListener("click",h=>{h.stopPropagation(),this.closeSettingsModal(!1)});//! OKボタン。
const m=document.getElementById("ok-btn");m&&m.addEventListener("click",h=>{h.stopPropagation(),this.closeSettingsModal(!0)})}closeSettingsModal(t){const e=document.getElementById("settings-modal");e&&e.remove()}destroy(){//! 音声を停止。
...
(a=document.getElementById("ok-btn"))==null||a.addEventListener("click",()=>{const l=document.getElementById("characterSpeed"),o=document.getElementById("effectiveSpeed"),c=document.getElementById("frequency-input"),u=document.getElementById("practiceDuration"),m=document.getElementById("groupSize"),h=document.getElementById("showInput"),v=parseInt(l.value,10);let f=parseInt(o.value,10);//! 実効速度は文字速度を上限とする。
f>v&&(f=v,o.value=v.toString()),this.settings.characterSpeed=v,this.settings.effectiveSpeed=f,this.settings.frequency=parseInt(c.value,10),this.settings.volume=parseInt(i.value,10)/100,this.settings.practiceDuration=parseInt(u.value,10),this.settings.groupSize=parseInt(m.value,10),this.settings.showInput=h.checked,this.saveSettings();//! AudioGeneratorを更新。
...
this.audio.stopPlaying()}}function et(p,t){const e=URL.createObjectURL(p),s=document.createElement("a");s.href=e,s.download=t,document.body.appendChild(s),s.click(),document.body.removeChild(s),URL.revokeObjectURL(e)}function st(p){return p.replace(/[^a-zA-Z0-9_-]/g,"_")}const H={characterSpeed:20,effectiveSpeed:15,frequency:700,volume:.7};class nt{constructor(){d(this,"audio");d(this,"settings",{...H});d(this,"state",{currentCategory:"qso",selectedTemplate:null,isPlaying:!1,userInput:"",showResult:!1,showAnswer:!1,showDialogFormat:!1});d(this,"customTemplates",[]);//! 設定を読み込む。
...
n.addEventListener("click",l=>{l.target===n&&n.remove()})}deleteCustomTemplate(t){confirm("この定型文を削除しますか?")&&(this.customTemplates=this.customTemplates.filter(e=>e.id!==t),this.saveCustomTemplates(),this.render())}showSettings(){var l,o,c;const t=`
			<div class="settings-modal active" id="settings-modal">
				<div class="settings-content">
					<h2>設定</h2>
		...
		`;document.body.insertAdjacentHTML("beforeend",t);//! スライダーの値変更を監視。
...
(l=document.getElementById("test-morse-btn"))==null||l.addEventListener("click",async()=>{//! 現在の設定値で一時的にAudioGeneratorを更新。
...
const u=I.textToMorse("CQ");await this.audio.playMorseString(u)});//! 設定の一時保存（キャンセル用）。
const a=()=>{this.loadSettings()};//! 保存ボタン。
(o=document.getElementById("save-btn"))==null||o.addEventListener("click",()=>{var u;this.settings.characterSpeed=parseInt(e.value),this.settings.effectiveSpeed=parseInt(s.value),this.settings.frequency=parseInt(i.value),this.settings.volume=parseFloat(n.value),this.saveSettings();//! AudioGeneratorを更新。
this.audio.updateSettings({frequency:this.settings.frequency,volume:this.settings.volume,wpm:this.settings.characterSpeed,effectiveWpm:this.settings.effectiveSpeed}),(u=document.getElementById("settings-modal"))==null||u.remove()});//! キャンセル。
(c=document.getElementById("cancel-btn"))==null||c.addEventListener("click",()=>{var u;a(),(u=document.getElementById("settings-modal"))==null||u.remove()});//! モーダル外クリックで閉じる（キャンセル扱い）。
const r=document.getElementById("settings-modal");r==null||r.addEventListener("click",u=>{u.target===r&&(a(),r.remove())})}render(){const t=document.getElementById("app");t&&(t.innerHTML=`
...
const m=[...M.getBuiltinTemplates(),...this.customTemplates].find(h=>h.id===c);m&&(this.state.selectedTemplate=m,this.state.showResult=!1,this.state.showAnswer=!1,this.state.showDialogFormat=!1,this.state.userInput="",this.render(),this.renderPracticeArea())}}})});//! 一覧に戻るボタン。
...
document.querySelectorAll(".edit-btn").forEach(o=>{o.addEventListener("click",()=>{const c=o.getAttribute("data-template-id");if(c){const u=this.customTemplates.find(m=>m.id===c);u&&this.showCustomTemplateDialog(u)}})});//! ユーザー定義定型文の削除ボタン。
...
//# sourceMappingURL=index-CkhHIgTi.js.map
