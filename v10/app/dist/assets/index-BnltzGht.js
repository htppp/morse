...
const s=P.calculate(this.currentWPM);//! トレーナーを初期化（コールバックで音声制御）。
...
n.addEventListener("mousedown",o=>{o.preventDefault(),this.isKeyPressed||this.handleKeyDown()}),n.addEventListener("mouseup",o=>{o.preventDefault(),this.isKeyPressed&&this.handleKeyUp()}),n.addEventListener("mouseleave",()=>{this.isKeyPressed&&this.handleKeyUp()});//! タッチイベント。
n.addEventListener("touchstart",o=>{o.preventDefault(),this.isKeyPressed||this.handleKeyDown()}),n.addEventListener("touchend",o=>{o.preventDefault(),this.isKeyPressed&&this.handleKeyUp()}),n.addEventListener("touchcancel",()=>{this.isKeyPressed&&this.handleKeyUp()})}//! モーダルのキャンセルボタン。
const i=document.getElementById("cancel-btn");i==null||i.addEventListener("click",()=>{this.closeSettingsModal(!1)});//! モーダルのOKボタン。
...
const t=document.getElementById("volume-range"),s=document.getElementById("volume-input"),n=document.getElementById("frequency-input"),i=document.getElementById("wpm-input"),a=Math.round(this.audio.getVolume()*100);t&&(t.value=a.toString()),s&&(s.value=a.toString()),n&&(n.value=this.audio.getFrequency().toString()),i&&(i.value=this.currentWPM.toString());//! 音量スライダーと数値入力の同期のみ（実際の音声設定は変更しない）。
const r=()=>{t&&s&&(s.value=t.value)},o=()=>{t&&s&&(t.value=s.value)};t==null||t.addEventListener("input",r),s==null||s.addEventListener("input",o);//! キーバインド設定。
...
const s=document.getElementById("volume-input"),n=document.getElementById("frequency-input"),i=document.getElementById("wpm-input"),a=document.getElementById("key-binding");if(s){const r=parseInt(s.value,10)/100;this.audio.setVolume(r)}if(n){const r=parseInt(n.value,10);this.audio.setFrequency(r)}if(i){const r=parseInt(i.value,10);this.currentWPM=r,this.audio.setWPM(r);//! WPMをlocalStorageに保存。
localStorage.setItem("verticalKeyWPM",r.toString());//! タイミングを再計算してトレーナーを再初期化。
const o=P.calculate(r);this.trainer=new $(this.buffer,this.timer,o,{onKeyPress:()=>{this.audio.startContinuousTone()},onKeyRelease:()=>{this.audio.stopContinuousTone()}});//! 現在のWPM表示を更新。
...
this.initializeTrainer()}initializeTrainer(){const e=P.calculate(this.currentWPM);this.trainer=new j(this.buffer,this.timer,e,{onElementStart:(t,s)=>{//! 要素送信開始時に指定時間だけ音を鳴らす。
...
const n=document.getElementById("left-paddle");n&&(n.addEventListener("mousedown",l=>{l.preventDefault(),this.handleLeftPaddlePress()}),n.addEventListener("mouseup",l=>{l.preventDefault(),this.handleLeftPaddleRelease()}),n.addEventListener("mouseleave",()=>{this.leftPressed&&this.handleLeftPaddleRelease()}),n.addEventListener("touchstart",l=>{l.preventDefault(),this.handleLeftPaddlePress()}),n.addEventListener("touchend",l=>{l.preventDefault(),this.handleLeftPaddleRelease()}),n.addEventListener("touchcancel",()=>{this.leftPressed&&this.handleLeftPaddleRelease()}));//! ボタンイベント（右パドル）。
const i=document.getElementById("right-paddle");i&&(i.addEventListener("mousedown",l=>{l.preventDefault(),this.handleRightPaddlePress()}),i.addEventListener("mouseup",l=>{l.preventDefault(),this.handleRightPaddleRelease()}),i.addEventListener("mouseleave",()=>{this.rightPressed&&this.handleRightPaddleRelease()}),i.addEventListener("touchstart",l=>{l.preventDefault(),this.handleRightPaddlePress()}),i.addEventListener("touchend",l=>{l.preventDefault(),this.handleRightPaddleRelease()}),i.addEventListener("touchcancel",()=>{this.rightPressed&&this.handleRightPaddleRelease()}));//! モーダルのキャンセルボタン。
...
this.updateIntervalId=window.setInterval(()=>{this.updateDisplay()},100)}updateDisplay(){const e=document.getElementById("morse-buffer"),t=document.getElementById("decoded-text"),s=document.getElementById("char-count");if(e){const n=this.trainer.getBuffer(),i=this.trainer.getSequence(),a=i?`${n} ${i}`:n;e.textContent=a||"（ここにモールス符号が表示されます）"}if(t){const n=this.trainer.getDecoded();t.textContent=n||"（ここにデコードされた文字が表示されます）"}if(s){const n=this.trainer.getDecoded();s.textContent=n.length.toString()}}openSettingsModal(){const e=document.getElementById("settings-modal");if(!e)return;//! モーダルを表示。
e.classList.add("active");//! 現在の設定値をinput要素に反映。
const t=document.getElementById("volume-range"),s=document.getElementById("volume-input"),n=document.getElementById("frequency-input"),i=document.getElementById("wpm-input"),a=document.getElementById("iambic-mode-select"),r=document.getElementById("paddle-layout-select"),o=Math.round(this.audio.getVolume()*100);t&&(t.value=o.toString()),s&&(s.value=o.toString()),n&&(n.value=this.audio.getFrequency().toString()),i&&(i.value=this.currentWPM.toString()),a&&(a.value=this.iambicMode),r&&(r.value=this.paddleLayout);//! 音量スライダーと数値入力の同期のみ（実際の音声設定は変更しない）。
const l=()=>{t&&s&&(s.value=t.value)},d=()=>{t&&s&&(t.value=s.value)};t==null||t.addEventListener("input",l),s==null||s.addEventListener("input",d);//! 左パドルキーバインド設定。
const m=document.getElementById("left-key-binding");m&&(m.addEventListener("click",()=>{m.value="キーを押してください...",m.classList.add("waiting-key")}),m.addEventListener("keydown",p=>{p.preventDefault(),p.stopPropagation(),m.value=p.code,m.classList.remove("waiting-key")}));//! 右パドルキーバインド設定。
const h=document.getElementById("right-key-binding");h&&(h.addEventListener("click",()=>{h.value="キーを押してください...",h.classList.add("waiting-key")}),h.addEventListener("keydown",p=>{p.preventDefault(),p.stopPropagation(),h.value=p.code,h.classList.remove("waiting-key")}))}closeSettingsModal(e){const t=document.getElementById("settings-modal");if(!t)return;//! モーダルを非表示。
if(t.classList.remove("active"),e){//! 設定を適用。
const s=document.getElementById("volume-input"),n=document.getElementById("frequency-input"),i=document.getElementById("wpm-input"),a=document.getElementById("iambic-mode-select"),r=document.getElementById("paddle-layout-select"),o=document.getElementById("left-key-binding"),l=document.getElementById("right-key-binding");if(s){const d=parseInt(s.value,10)/100;this.audio.setVolume(d)}if(n){const d=parseInt(n.value,10);this.audio.setFrequency(d)}if(i){const d=parseInt(i.value,10);this.currentWPM=d,this.audio.setWPM(d);//! WPMをlocalStorageに保存。
...
return i.length<6?(console.warn(`Line ${n+2} has insufficient columns, skipping`),null):{tags:i[0].trim(),frequency:parseInt(i[1].trim(),10)||1,abbreviation:i[2].trim(),english:i[3].trim(),japanese:i[4].trim(),description:(r=i[5])==null?void 0:r.trim(),example:(o=i[6])==null?void 0:o.trim()}}).filter(s=>s!==null)}//! 画面の種類。
//! 入力タイプ。
//! 設定項目の定義。
//! 設定値の型。
//! モーダルのコールバック関数。
class V{constructor(e,t,s,n){c(this,"modalId");c(this,"items");c(this,"callbacks");c(this,"currentValues");c(this,"initialValues");this.modalId=e,this.items=t,this.currentValues={...s},this.initialValues={...s},this.callbacks=n}show(e){//! この画面用の設定項目をフィルタリングして優先度順にソート。
...
const r=document.getElementById(`${this.modalId}-${a.key}`);r==null||r.addEventListener("click",()=>{var o,l;(l=(o=this.callbacks).onTestPlay)==null||l.call(o)})}});//! 保存ボタン。
...
t.addEventListener("click",a=>{a.target===t&&(this.currentValues={...this.initialValues},this.callbacks.onCancel(),this.close())})}close(){const e=document.getElementById(this.modalId);e==null||e.remove()}}const Q=[{screens:["flashcard","koch","horizontal-key","vertical-key"],priority:10,key:"volume",label:"音量",inputType:"range-with-number",min:0,max:100,step:1,unit:"%"},{screens:["flashcard","koch","horizontal-key","vertical-key"],priority:20,key:"frequency",label:"周波数 (Hz)",inputType:"number",min:400,max:1200,step:50},{screens:["flashcard","koch","horizontal-key","vertical-key"],priority:30,key:"wpm",label:"WPM (速度: 5-40)",inputType:"number",min:5,max:40,step:1},{screens:["listening"],priority:5,key:"characterSpeed",label:"文字速度 (WPM)",inputType:"number",min:5,max:40,step:1},{screens:["listening"],priority:6,key:"effectiveSpeed",label:"実効速度 (WPM)",inputType:"number",min:5,max:40,step:1},{screens:["listening"],priority:20,key:"frequency",label:"周波数 (Hz)",inputType:"number",min:400,max:1e3,step:10},{screens:["listening"],priority:10,key:"volume",label:"音量",inputType:"range-with-number",min:0,max:100,step:5,unit:"%"},{screens:["flashcard","listening"],priority:100,key:"testPlay",label:"テスト再生",inputType:"button",buttonText:"再生",buttonClass:"test-button"}];class te{constructor(){c(this,"allEntries",[]);c(this,"filteredEntries",[]);c(this,"currentState","loading");c(this,"selectedTags",new Set);c(this,"selectedFrequencies",new Set([5]));c(this,"searchQuery","");c(this,"displayMode","card");c(this,"sortColumn","abbreviation");c(this,"sortDirection","asc");c(this,"learnQuestionType","abbr-to-meaning");c(this,"learnCards",[]);c(this,"currentLearnIndex",0);c(this,"isFlipped",!1);c(this,"reviewMode",!1);c(this,"progress",{known:new Set,unknown:new Set});c(this,"questionType","abbr-to-meaning");c(this,"questionCount",10);c(this,"questions",[]);c(this,"currentQuestionIndex",0);c(this,"results",[]);c(this,"audio");c(this,"currentlyPlaying",null);this.audio=new T({frequency:700,volume:.5,wpm:20});//! ライブラリから状態を読み込む。
this.progress=f.loadProgress();const e=f.loadFilters();this.selectedTags=e.selectedTags,this.selectedFrequencies=e.selectedFrequencies,this.searchQuery=e.searchQuery;const t=f.loadViewState();this.displayMode=t.displayMode,this.sortColumn=t.sortColumn,this.sortDirection=t.sortDirection,this.learnQuestionType=t.learnQuestionType,this.questionType=t.examQuestionType}saveProgress(){f.saveProgress(this.progress)}clearProgress(){this.progress={known:new Set,unknown:new Set},f.clearProgress()}saveFilters(){f.saveFilters({selectedTags:this.selectedTags,selectedFrequencies:this.selectedFrequencies,searchQuery:this.searchQuery})}async render(){const e=document.getElementById("app");if(e)if(this.currentState==="loading"){//! ローディング画面を表示。
e.innerHTML=`
				<div class="container">
					<header class="header">
						<h1>CW略語・Q符号学習</h1>
...
const s=f.loadViewState();this.currentState=s.viewMode,this.render()}catch(s){console.error("Failed to load flashcard data:",s),e.innerHTML=`
					<div class="container">
						<header class="header">
							<h1>CW略語・Q符号学習</h1>
							<button class="back-btn">メニューに戻る</button>
...
const t=document.getElementById("toggle-display-btn");t&&t.addEventListener("click",()=>{this.displayMode="list",f.saveDisplayMode(this.displayMode),this.renderEntries()})}renderListView(e){e.innerHTML=`
			<div class="entries-header">
				<h3>略語一覧 (${this.filteredEntries.length}件)</h3>
...
const n=document.getElementById("toggle-display-btn");n&&n.addEventListener("click",()=>{this.displayMode="card",f.saveDisplayMode(this.displayMode),this.renderEntries()})}renderLearnMode(){if(this.learnCards.length===0){//! セットアップ画面を表示。
...
document.querySelectorAll(".question-type-btn").forEach(o=>{o.addEventListener("click",()=>{const l=o.getAttribute("data-type");l&&(this.learnQuestionType=l,f.saveLearnQuestionType(this.learnQuestionType),this.renderLearnSetup())})});//! 学習開始ボタン。
...
(t.type==="morse-to-abbr"||t.type==="morse-to-meaning")&&setTimeout(()=>this.playMorse(t.entry.abbreviation),500)}renderExamResultMode(){const e=document.getElementById("app");if(!e)return;const t=E.calculateScore(this.results),s=E.isPassed(t.percentage),n=E.getWrongAnswers(this.results);e.innerHTML=`
			<div class="container">
				<header class="header">
					<h1>CW略語・Q符号学習 - 結果</h1>
					<button class="back-btn">メニューに戻る</button>
...
document.querySelectorAll(".question-type-btn").forEach(r=>{r.addEventListener("click",()=>{const o=r.getAttribute("data-type");o&&(this.questionType=o,f.saveExamQuestionType(this.questionType),this.renderExamSetup())})});//! 問題数ボタン。
...
document.querySelectorAll(".tab-button").forEach(n=>{n.addEventListener("click",()=>{const i=n.getAttribute("data-tab");i==="browse"?(this.currentState="browse",f.saveViewMode("browse"),this.render()):i==="learn"?(this.currentState="learn",f.saveViewMode("learn"),this.render()):i==="exam"&&(this.questions=[],this.results=[],this.currentQuestionIndex=0,this.currentState="exam",f.saveViewMode("exam"),this.render())})})}updateFilteredCount(){const e=document.getElementById("filtered-count");e&&(e.textContent=this.filteredEntries.length.toString());//! 問題数の最大値も更新（試験モードの場合）。
...
f.saveSortState(this.sortColumn,this.sortDirection),this.updateFilteredEntries(),this.renderEntries()}getSortIndicator(e){return this.sortColumn!==e?"":this.sortDirection==="asc"?" ▲":" ▼"}formatAbbreviation(e){const t=e.match(/^\[([A-Z]+)\]$/);return t?`<span class="prosign">${t[1]}</span>`:e}async playMorse(e){try{//! 既に再生中の場合は停止。
...
const s=new V("flashcard-settings-modal",Q,e,{onSave:n=>{//! 設定を保存。
...
const R=["K","M","U","R","E","S","N","A","P","T","L","W","I",".","J","Z","=","F","O","Y","V",",","G","5","/","Q","9","2","H","3","8","B","?","4","7","C","1","D","6","0","X"];class se{constructor(){c(this,"audio");c(this,"viewMode","learning");c(this,"settings",{...F});c(this,"state",{currentLesson:1,isPlaying:!1,userInput:"",correctAnswer:"",groups:[],currentGroupIndex:0});c(this,"customState",{selectedChars:new Set,isCustomRunning:!1,customUserInput:"",customCorrectAnswer:"",customGroups:[],customCurrentGroupIndex:0,customIsPlaying:!1});//! 設定を読み込む。
...
const s=document.getElementById("volume-range"),n=document.getElementById("volume-input");s&&n&&(s.oninput=()=>{n.value=s.value},n.oninput=()=>{s.value=n.value});//! 設定を復元する関数。
const i=()=>{this.settings={...e},this.audio.updateSettings({frequency:e.frequency,volume:e.volume,wpm:e.characterSpeed,effectiveWpm:e.effectiveSpeed})};//! OK（保存）。
(a=document.getElementById("ok-btn"))==null||a.addEventListener("click",()=>{const o=document.getElementById("characterSpeed"),l=document.getElementById("effectiveSpeed"),d=document.getElementById("frequency-input"),m=document.getElementById("practiceDuration"),h=document.getElementById("groupSize"),p=document.getElementById("showInput"),v=parseInt(o.value,10);let y=parseInt(l.value,10);//! 実効速度は文字速度を上限とする。
y>v&&(y=v,l.value=v.toString()),this.settings.characterSpeed=v,this.settings.effectiveSpeed=y,this.settings.frequency=parseInt(d.value,10),this.settings.volume=parseInt(n.value,10)/100,this.settings.practiceDuration=parseInt(m.value,10),this.settings.groupSize=parseInt(h.value,10),this.settings.showInput=p.checked,this.saveSettings();//! AudioGeneratorを更新。
...
(r=document.getElementById("cancel-btn"))==null||r.addEventListener("click",()=>{i(),t.remove()});//! モーダル外クリックで閉じる（キャンセル扱い）。
t.addEventListener("click",o=>{o.target===t&&(i(),t.remove())})}render(){const e=document.getElementById("app");e&&(e.innerHTML=`
...
this.audio.stopPlaying()}}function ne(u,e){const t=URL.createObjectURL(u),s=document.createElement("a");s.href=t,s.download=e,document.body.appendChild(s),s.click(),document.body.removeChild(s),URL.revokeObjectURL(t)}function ie(u){return u.replace(/[^a-zA-Z0-9_-]/g,"_")}const H={characterSpeed:20,effectiveSpeed:15,frequency:700,volume:.7};class ae{constructor(){c(this,"audio");c(this,"settings",{...H});c(this,"state",{currentCategory:"qso",selectedTemplate:null,isPlaying:!1,userInput:"",showResult:!1,showAnswer:!1,showDialogFormat:!1});c(this,"customTemplates",[]);//! 設定を読み込む。
...
const s=new V("listening-settings-modal",Q,e,{onSave:n=>{//! 実効速度は文字速度を上限とする。
...
const n=I.textToMorse("CQ");await this.audio.playMorseString(n)}});//! モーダルを表示。
...
const h=[...B.getBuiltinTemplates(),...this.customTemplates].find(p=>p.id===d);h&&(this.state.selectedTemplate=h,this.state.showResult=!1,this.state.showAnswer=!1,this.state.showDialogFormat=!1,this.state.userInput="",this.render(),this.renderPracticeArea())}}})});//! 一覧に戻るボタン。
...
document.querySelectorAll(".edit-btn").forEach(l=>{l.addEventListener("click",()=>{const d=l.getAttribute("data-template-id");if(d){const m=this.customTemplates.find(h=>h.id===d);m&&this.showCustomTemplateDialog(m)}})});//! ユーザー定義定型文の削除ボタン。
...
this.routes=[{path:"",view:O},{path:"menu",view:O},{path:"vertical",view:X},{path:"horizontal",view:Z},{path:"flashcard",view:te},{path:"koch",view:se},{path:"listening",view:ae}]}init(){//! ハッシュ変更時のリスナーを登録。
...
//# sourceMappingURL=index-BnltzGht.js.map
