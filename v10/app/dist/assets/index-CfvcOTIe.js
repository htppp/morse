...
//! 設定値の型。
//! モーダルのコールバック関数。
class C{constructor(t,e,s,n){this.modalId=t,this.items=e,this.currentValues={...s},this.initialValues={...s},this.callbacks=n}show(t){//! この画面用の設定項目をフィルタリングして優先度順にソート。
...
const s=new C("vertical-key-settings-modal",w,t,{onSave:n=>{//! 設定を保存。
...
const s=new C("horizontal-key-settings-modal",w,t,{onSave:n=>{//! 設定を保存。
			...
			<div class="filter-section">
				...
				<div class="filter-group">
					...
					<div class="tag-filter" id="tag-filter">
						${f.getAllTags(this.allEntries).map(e=>`
...
(e.type==="morse-to-abbr"||e.type==="morse-to-meaning")&&setTimeout(()=>this.playMorse(e.entry.abbreviation),500)}renderExamResultMode(){const t=document.getElementById("app");if(!t)return;const e=f.calculateScore(this.results),s=f.isPassed(e.percentage),n=f.getWrongAnswers(this.results);t.innerHTML=`
			<div class="container">
				<header class="header">
					<h1>CW略語・Q符号学習 - 結果</h1>
					<button class="back-btn">メニューに戻る</button>
...
t=f.filterByTags(t,this.selectedTags);//! 使用頻度でフィルタリング。
t=f.filterByFrequencies(t,this.selectedFrequencies);//! 検索クエリでフィルタリング。
t=f.filterByQuery(t,this.searchQuery);//! ソート適用。
this.filteredEntries=this.sortEntries(t)}sortEntries(t){const e=this.sortDirection==="asc";switch(this.sortColumn){case"abbreviation":return f.sortByAbbreviation(t,e);case"english":return f.sortByEnglish(t,e);case"japanese":return f.sortByJapanese(t,e);case"frequency":return f.sortByFrequency(t,e);case"tags":return f.sortByTags(t,e);default:return t}}toggleSort(t){if(this.sortColumn===t){//! 同じ列なら方向を反転。
...
const e=E.textToMorse(t);if(e){//! シンプルな再生実装（scheduleToneを使用）。
...
this.questions=f.generateExamQuestions(this.filteredEntries,this.questionType,e),this.currentQuestionIndex=0,this.results=[],this.render()}handleAnswer(t){const e=this.questions[this.currentQuestionIndex],s=f.checkAnswer(e,t);//! 結果を記録。
...
const s=new C("flashcard-settings-modal",w,t,{onSave:n=>{//! 設定を保存。
...
const s=new C("koch-settings-modal",w,t,{onSave:n=>{//! 実効速度は文字速度を上限とする。
...
const t={characterSpeed:this.settings.characterSpeed,effectiveSpeed:this.settings.effectiveSpeed,frequency:this.settings.frequency,volume:Math.round(this.settings.volume*100)};//! 設定変更前の値を保存（キャンセル時の復元用）。
const e={...this.settings};//! SettingsModalを作成。
const s=new C("listening-settings-modal",w,t,{onSave:n=>{//! 実効速度は文字速度を上限とする。
let i=n.effectiveSpeed;const a=n.characterSpeed;i>a&&(i=a);//! 設定を保存。
...
this.settings={...e},this.audio.updateSettings({frequency:e.frequency,volume:e.volume,wpm:e.characterSpeed,effectiveWpm:e.effectiveSpeed})},onTestPlay:async()=>{//! テスト再生。
...
const t=E.textToMorse(this.state.selectedTemplate.content);//! WAVファイルを生成。
...
//# sourceMappingURL=index-CfvcOTIe.js.map
