...
this.audio.stopPlaying()}}const H={characterSpeed:20,effectiveSpeed:15,frequency:700,volume:.7};class et{constructor(){d(this,"audio");d(this,"settings",{...H});d(this,"state",{currentCategory:"qso",selectedTemplate:null,isPlaying:!1,userInput:"",showResult:!1,showAnswer:!1,showDialogFormat:!1});d(this,"customTemplates",[]);//! 設定を読み込む。
...
n.addEventListener("click",l=>{l.target===n&&n.remove()})}deleteCustomTemplate(t){confirm("この定型文を削除しますか?")&&(this.customTemplates=this.customTemplates.filter(e=>e.id!==t),this.saveCustomTemplates(),this.render())}showSettings(){var r,l;const t=document.createElement("div");t.className="modal-overlay",t.innerHTML=`
...
(l=document.getElementById("cancel-btn"))==null||l.addEventListener("click",()=>{a(),t.remove()});//! モーダル外クリックで閉じる（キャンセル扱い）。
t.addEventListener("click",o=>{o.target===t&&(a(),t.remove())})}render(){const t=document.getElementById("app");t&&(t.innerHTML=`
...
const s=URL.createObjectURL(e),i=document.createElement("a");i.href=s,i.download=`${this.state.selectedTemplate.title.replace(/[^a-zA-Z0-9]/g,"_")}.wav`,document.body.appendChild(i),i.click(),document.body.removeChild(i),URL.revokeObjectURL(s)}catch(t){console.error("WAVダウンロードエラー:",t),alert("WAVファイルの生成に失敗しました。")}}destroy(){//! AudioGeneratorを停止。
this.audio.stopPlaying()}}class st{constructor(){d(this,"currentView",null);d(this,"routes",[]);//! ルート定義。
this.routes=[{path:"",view:O},{path:"menu",view:O},{path:"vertical",view:z},{path:"horizontal",view:j},{path:"flashcard",view:_},{path:"koch",view:tt},{path:"listening",view:et}]}init(){//! ハッシュ変更時のリスナーを登録。
