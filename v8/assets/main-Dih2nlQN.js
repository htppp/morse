import{A as I,M as v}from"./morse-code-CFHo7w6l.js";const l="K M U R E S N A P T L W I . J Z = F O Y , V G 5 / Q 9 2 H 3 8 B ? 4 7 C 1 D 6 0 X".split(" ");function u(d){return l.slice(0,d+1)}function g(d,t=5,e=60){const s=[],i=Math.floor(e*20/(t*5));for(let n=0;n<i;n++){let r="";for(let a=0;a<t;a++){const c=d[Math.floor(Math.random()*d.length)];r+=c}s.push(r)}return s}const m=class m{static get(t){return this.settings[t]}static set(t,e){this.settings[t]=e,this.save()}static load(){try{const t=localStorage.getItem("v4.koch.settings");t&&(this.settings={...this.defaultSettings,...JSON.parse(t)})}catch(t){console.error("Failed to load Koch settings:",t)}}static save(){try{localStorage.setItem("v4.koch.settings",JSON.stringify(this.settings))}catch(t){console.error("Failed to save Koch settings:",t)}}static getAll(){return{...this.settings}}static reset(){this.settings={...this.defaultSettings},this.save()}};m.defaultSettings={characterSpeed:20,effectiveSpeed:20,frequency:750,volume:.7,groupSize:9,displayMode:"fixed",practiceDuration:60,showInput:!0},m.settings={...m.defaultSettings};let o=m;class C{constructor(){this.viewMode="learning",this.state={currentLesson:1,isPlaying:!1,userInput:"",correctAnswer:"",groups:[],currentGroupIndex:0},this.customState={selectedChars:new Set,isCustomRunning:!1,customUserInput:"",customCorrectAnswer:"",customGroups:[],customCurrentGroupIndex:0,customIsPlaying:!1},o.load();const t=o.getAll();this.audioSystem=new I({frequency:t.frequency,volume:t.volume,wpm:t.characterSpeed}),this.loadProgress(),this.render()}loadProgress(){try{const t=localStorage.getItem("v4.koch.currentLesson");t&&(this.state.currentLesson=parseInt(t))}catch(t){console.error("Failed to load progress:",t)}}saveProgress(){try{localStorage.setItem("v4.koch.currentLesson",this.state.currentLesson.toString())}catch(t){console.error("Failed to save progress:",t)}}async startLesson(){const t=o.getAll(),e=u(this.state.currentLesson);this.state.groups=g(e,5,t.practiceDuration),this.state.currentGroupIndex=0,this.state.userInput="",this.state.correctAnswer=this.state.groups.join(""),this.state.isPlaying=!1,this.audioSystem.updateSettings({frequency:t.frequency,volume:t.volume,wpm:t.characterSpeed,effectiveWpm:t.effectiveSpeed}),this.renderPractice()}async playMorse(){if(!this.state.isPlaying){this.state.isPlaying=!0,this.state.currentGroupIndex=0,this.updateProgress(),this.updatePlaybackButtons();for(let t=0;t<this.state.groups.length&&this.state.isPlaying;t++){const e=this.state.groups[t],s=v.textToMorse(e);await this.audioSystem.playMorseString(s+" /"),this.state.currentGroupIndex=t+1,this.updateProgress()}this.state.isPlaying=!1,this.updatePlaybackButtons(),this.state.currentGroupIndex>=this.state.groups.length&&this.showResult()}}pauseMorse(){this.state.isPlaying=!1,this.audioSystem.stopPlaying(),this.updatePlaybackButtons()}stopLesson(){this.state.isPlaying=!1,this.audioSystem.stopPlaying(),this.render()}showResult(){const t=this.calculateAccuracy(),e=t>=90,s=document.getElementById("resultContainer");s&&(s.innerHTML=`
    ...
    `,e&&this.state.currentLesson<40&&(this.state.currentLesson++,this.saveProgress()))}calculateAccuracy(){if(!this.state.userInput)return 0;const t=this.state.correctAnswer.replace(/\s/g,""),e=this.state.userInput.replace(/\s/g,""),s=Math.max(t.length,e.length);let i=0;for(let n=0;n<s;n++)t[n]===e[n]&&i++;return i/s*100}updateProgress(){const t=document.getElementById("lessonProgress"),e=document.getElementById("progressBar");if(t&&e){const s=this.state.currentGroupIndex/this.state.groups.length*100;t.textContent=`進行: ${this.state.currentGroupIndex}/${this.state.groups.length} (${s.toFixed(0)}%)`,e.style.width=`${s}%`}this.updatePlaybackButtons()}showSettings(){const t={...o.getAll()},e=o.getAll(),s=document.createElement("div");s.className="modal",s.innerHTML=`
    ...
    `,document.body.appendChild(s);const i=document.getElementById("volumeRange"),n=document.getElementById("volumeInput");i&&n&&(i.oninput=a=>{const c=a.target.value;n.value=c},n.oninput=a=>{const c=a.target.value;i.value=c});const r=()=>{o.set("characterSpeed",t.characterSpeed),o.set("effectiveSpeed",t.effectiveSpeed),o.set("frequency",t.frequency),o.set("volume",t.volume),o.set("practiceDuration",t.practiceDuration),o.set("groupSize",t.groupSize),o.set("showInput",t.showInput),this.audioSystem.updateSettings({frequency:t.frequency,volume:t.volume,wpm:t.characterSpeed})};document.getElementById("saveSettings")?.addEventListener("click",()=>{const a=document.getElementById("characterSpeed"),c=document.getElementById("effectiveSpeed"),y=document.getElementById("frequency"),b=document.getElementById("practiceDuration"),f=document.getElementById("groupSize"),S=document.getElementById("showInput"),h=parseInt(a.value);let p=parseInt(c.value);p>h&&(p=h,c.value=h.toString()),o.set("characterSpeed",h),o.set("effectiveSpeed",p),o.set("frequency",parseInt(y.value)),o.set("volume",parseInt(n.value)/100),o.set("practiceDuration",parseInt(b.value)),o.set("groupSize",parseInt(f.value)),o.set("showInput",S.checked),this.audioSystem.updateSettings({frequency:o.get("frequency"),volume:o.get("volume"),wpm:o.get("characterSpeed"),effectiveWpm:o.get("effectiveSpeed")}),s.remove(),this.state.groups.length>0&&this.renderPractice()}),document.getElementById("cancelSettings")?.addEventListener("click",()=>{r(),s.remove()}),document.getElementById("closeSettings")?.addEventListener("click",()=>{r(),s.remove()}),s.addEventListener("click",a=>{a.target===s&&(r(),s.remove())})}render(){const t=document.getElementById("app");if(!t)return;u(this.state.currentLesson),l.slice(0,40).map((s,i)=>{const n=i+1,r=u(n),a=n===this.state.currentLesson,c=n<this.state.currentLesson;return`
    ...
    `,document.getElementById("backBtn")?.addEventListener("click",()=>{window.location.href="./index.html"}),document.getElementById("settingsIcon")?.addEventListener("click",()=>{this.showSettings()}),document.querySelectorAll(".tab-button").forEach(s=>{s.addEventListener("click",()=>{const i=s.getAttribute("data-tab");i&&(this.viewMode=i,this.render())})}),this.setupModeEventListeners()}renderModeContent(){switch(this.viewMode){case"learning":return this.renderLearningMode();case"custom":return this.renderCustomMode();default:return this.renderLearningMode()}}renderLearningMode(){const t=u(this.state.currentLesson),e=l.slice(0,40).map((s,i)=>{const n=i+1,r=u(n),a=n===this.state.currentLesson,c=n<this.state.currentLesson;return`
        <div class="lesson-item ${a?"current":""} ${c?"passed":""}" data-lesson="${n}">
          <div class="lesson-num">L${n}</div>
          <div class="lesson-chars">${r.join("")}</div>
        </div>
    ...
    `;const i=document.getElementById("userInput");i&&i.addEventListener("input",n=>{this.state.userInput=n.target.value.toUpperCase()}),document.getElementById("playBtn")?.addEventListener("click",()=>{this.playMorse()}),document.getElementById("pauseBtn")?.addEventListener("click",()=>{this.pauseMorse()}),document.getElementById("stopBtn")?.addEventListener("click",()=>{this.stopLesson()}),this.setupKeyboardEvents(s),this.updatePlaybackButtons()}updatePlaybackButtons(){const t=document.getElementById("playBtn"),e=document.getElementById("pauseBtn");t&&e&&(this.state.isPlaying?(t.disabled=!0,e.disabled=!1):(t.disabled=!1,e.disabled=!0))}renderKeyboard(t){const e=o.get("groupSize"),s=[];for(let i=0;i<l.length;i+=e)s.push(l.slice(i,i+e));return`
      <div class="keyboard">
            ...
            ${s.map((i,n)=>`
              <div class="keyboard-group">
                ...
                <div class="group-keys">
                  ${i.map(r=>{const a=t.includes(r);return`
    ...
    `}setupKeyboardEvents(t){const e=document.getElementById("userInput");e&&(document.querySelectorAll(".key-btn:not(.special)").forEach(s=>{s.addEventListener("click",i=>{i.preventDefault();const n=i.target.getAttribute("data-char");n&&t.includes(n)&&(e.value+=n,this.state.userInput=e.value.toUpperCase())})}),document.getElementById("spaceBtn")?.addEventListener("click",s=>{s.preventDefault(),e.value+=" ",this.state.userInput=e.value.toUpperCase()}),document.getElementById("backspaceBtn")?.addEventListener("click",s=>{s.preventDefault(),e.value=e.value.slice(0,-1),this.state.userInput=e.value.toUpperCase()}),e.addEventListener("keydown",s=>{s.key===""&&(s.preventDefault(),e.value+="",this.state.userInput=e.value.toUpperCase())}))}async startCustom(){const t=o.getAll(),e=Array.from(this.customState.selectedChars);this.customState.customGroups=g(e,5,t.practiceDuration),this.customState.customCurrentGroupIndex=0,this.customState.customUserInput="",this.customState.customCorrectAnswer=this.customState.customGroups.join(""),this.customState.customIsPlaying=!1,this.customState.isCustomRunning=!0,this.audioSystem.updateSettings({frequency:t.frequency,volume:t.volume,wpm:t.characterSpeed,effectiveWpm:t.effectiveSpeed}),this.render(),this.renderCustomPractice()}renderCustomPractice(){const t=document.getElementById("customPracticeContainer");if(!t)return;const e=o.getAll();t.innerHTML=`
      <div class="practice-area">
        <div class="progress-section">
          <div class="progress-bar-container">
